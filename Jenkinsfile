pipeline {
    ...
    stages {
		...
		stage("Trigger CD Pipeline") {
			steps {
				script {
					sh "curl -v -k --user noreply:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-ec2-34-229-16-53.compute-1.amazonaws.com:8080/job/gitops-register-app-cd/buildWithParameters?token=gitops-token'"
				}
			}
		}
    }
